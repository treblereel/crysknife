...
@Page(path = "SingleSelectDemo")
@Singleton
@Templated("SingleSelectDemo.html")
public class SingleSelectDemo implements IsElement<HTMLDivElement> {

  @DataField
  SingleSelect basicSelect = singleSelect("Select a value")
      .add(menu(select, single)
          .addContent(menuContent()
              .addList(menuList()
                  .add(menuItem("opt-1", "Option 1"))
                  .add(menuItem("opt-2", "Option 2"))
                  .add(menuItem("opt-3", "Option 3"))
                  .add(menuItem("opt-4", "Option 4")))));
}
