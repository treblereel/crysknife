 ...
 * @param <R> type of response the callback expects. Use {@link Void} for methods returning
 *        {@code void}.
 */
public interface RemoteCallback<R> {

  /**
   * Invoked by the RPC proxy after the remote method has been executed and its response has been
   * received.
   *
   * @param response the response returned from the remote call. Will be null if the remote method
   *        returns null or has a void return type.
   */
